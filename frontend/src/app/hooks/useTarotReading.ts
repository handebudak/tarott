...
export const useTarotReading = ({ enableReversed, type }: UseTarotReadingProps) => {
  ...
  const handleCardSelect = (cardIndex: number) => {
    ...
    if (type === 'single') {
      ...
      // Kart seçildiğinde seçilen kart bölümüne kaydır
      setTimeout(() => {
        const selectedCardElement = document.querySelector('[data-selected-card-section]');
        if (selectedCardElement) {
          const headerHeight = 80; // Header'ın yaklaşık yüksekliği
          const elementTop = selectedCardElement.getBoundingClientRect().top + window.pageYOffset;
          window.scrollTo({
            top: elementTop - headerHeight - 20, // 20px ekstra boşluk
            behavior: 'smooth'
          });
        }
      }, 100);
      ...
      if (selectedCards.includes(cardIndex)) {
        ...
        // Üçüncü kart seçildiğinde seçilen kartlar bölümüne kaydır
        if (selectedCards.length === 2) {
          setTimeout(() => {
            const selectedCardsElement = document.querySelector('[data-selected-cards-section]');
            if (selectedCardsElement) {
              const headerHeight = 80; // Header'ın yaklaşık yüksekliği
              const elementTop = selectedCardsElement.getBoundingClientRect().top + window.pageYOffset;
              window.scrollTo({
                top: elementTop - headerHeight - 20, // 20px ekstra boşluk
                behavior: 'smooth'
              });
  ...
  const handleRandomSelect = () => {
    ...
    if (type === 'single') {
      ...
      // Rastgele seçim yapıldığında seçilen kart bölümüne kaydır
      setTimeout(() => {
        const selectedCardElement = document.querySelector('[data-selected-card-section]');
        if (selectedCardElement) {
          const headerHeight = 80; // Header'ın yaklaşık yüksekliği
      ...
      // Rastgele seçim yapıldığında seçilen kartlar bölümüne kaydır
      setTimeout(() => {
        const selectedCardsElement = document.querySelector('[data-selected-cards-section]');
        if (selectedCardsElement) {
          const headerHeight = 80; // Header'ın yaklaşık yüksekliği
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Fal sonucu geldiğinde fal başlığına kaydır
      setTimeout(() => {
        const readingElement = document.querySelector('[data-reading-section]');
        if (readingElement) {
          const headerHeight = 80; // Header'ın yaklaşık yüksekliği
          const elementTop = readingElement.getBoundingClientRect().top + window.pageYOffset;
          window.scrollTo({
            top: elementTop - headerHeight - 20, // 20px ekstra boşluk
            behavior: 'smooth'
          });
  ...
  const getSelectedCardImage = (cardIndex: number) => {
    if (cards.length === 0) return "/card-bg.png";
    
    const card = cards[cardIndex];
    if (!card) return "/card-bg.png";
    
    console.log('Selected card:', card);
    
    // API'den gelen imagePath'i public klasöründeki dosya yoluna çevir
  ...
  const handleResetPage = () => {
    ...
    setReading(null);
    setIsLoading(false);
    setEnableReversedState(enableReversed); // enableReversedState'i sıfırla
  ...
  return {
    ...
    reading,
    cardOrientations,
    enableReversedState, // enableReversedState'i döndür
    modalConfig,
    setModalConfig,
